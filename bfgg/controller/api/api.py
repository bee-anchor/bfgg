...
def clone():
    ...
    grp = result["group"]
    OUTGOING_QUEUE.put(
        OutgoingMessageGrouped(CLONE, repo.encode("utf-8"), group=grp.encode("utf-8"))
    )
...
def results():
    ...
    getter = ReportHandler(
        results_folder, gatling_location, s3_bucket, s3_region, group=grp
    )
    url = getter.run()
    return {"Results": url}
