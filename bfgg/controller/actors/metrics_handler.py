...
class MetricsHandler:
    def __init__(self, results_folder: str, logger=logging.getLogger(__name__)):
        ...
        self.total_successes_count = Counter(
            "gatling_success_responses",
            "Total number of 'OK' responses",
            labelnames=["request_name", "group"],
        )
        self.total_failures_count = Counter(
            "gatling_failure_responses",
            "Total number of 'KO' responses",
            labelnames=["request_name", "group"],
        )
        self.total_errors_count = Counter(
            "gatling_errors", "Total number of error lines in the logs"
        )
        self.total_requests_count = Counter(
            "gatling_requests",
            "Total number of requests",
            labelnames=["request_name", "group"],
        )
        self.response_time_histo = Histogram(
            "gatling_response_times",
            "Histogram response times for all requests",
            labelnames=["request_name", "group"],
            unit="ms",
            buckets=[200, 500, 1000, 2000, 10000, float("inf")],
        )
        self.response_time_count = Counter(
            "gatling_response_times",
    ...
    def _create_metrics(self, log_line: str, group: str):
        try:
            if log_line.startswith("REQUEST"):
                ...
                self.response_time_histo.labels(request_name=name, group=group).observe(
                    response_time
                )
                self.response_time_count.labels(request_name=name, group=group).inc(
                    response_time
                )
                if status == "OK":
                    self.total_successes_count.labels(
                        request_name=name, group=group
                    ).inc()
                else:
                    self.total_failures_count.labels(
                        request_name=name, group=group
                    ).inc()
