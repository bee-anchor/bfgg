...
def clone_repo(project: str, tests_location: str):
    ...
    try :
        ...
        handle_state_change(status=Statuses.ERROR, extra_info="Exception found when cloning. Please make sure the "
                                                              "directory for cloning repositories exists.")
        return
    ...
    if "Receiving objects: 100%" in stderror:
        ...
        handle_state_change(status=Statuses.ERROR, extra_info="Could not read from remote repository. Check agent for "
                                                              "further details.")
