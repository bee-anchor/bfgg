...
def test_log_follower_get_current_logfile(mocker):
    ...
    mock_max = mocker.patch(
        "bfgg.agent.actors.log_follower.max", return_value=folders[0]
    )
    mocker.patch(
        "bfgg.agent.actors.log_follower.datetime",
...
def test_log_follower_get_current_logfile_folder_doesnt_exist(mocker):
    ...
    mock_max = mocker.patch(
        "bfgg.agent.actors.log_follower.max", side_effect=[ValueError, folders[0]]
    )
    mocker.patch(
        "bfgg.agent.actors.log_follower.datetime",
        **{"now.return_value.timestamp.return_value": 5},
