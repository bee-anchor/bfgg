...
class ReportHandlerTest(unittest.TestCase):
    ...
    def test_report_handler(self, datetime_mock, boto3_mock, os_mock, subprocess_mock):
        ...
        self.assertIn(
            [f'{gatling_folder}/bin/gatling.sh', '-ro', results_folder],
            subprocess_mock.Popen.call_args[0]
        )

        boto3_put_object_call_args = boto3_mock.resource.return_value.Bucket.return_value.put_object.call_args_list
        self.assertEqual(
            {'Key': f'NOW/1.html', 'Body': 'aabbcc', 'ACL': 'private', 'ContentType': 'text/html'},
            boto3_put_object_call_args[0][1]
        )
        self.assertEqual(
            {'Key': f'NOW/2/2.png', 'Body': 'aabbcc', 'ACL': 'private', 'ContentType': 'image/png'},
            boto3_put_object_call_args[1][1]
        )
        self.assertEqual(
            {'Key': f'NOW/3/3.json', 'Body': 'aabbcc', 'ACL': 'private', 'ContentType': 'application/json'},
            boto3_put_object_call_args[2][1]
        )

        self.assertEqual(f'https://{bucket}.s3.amazonaws.com/NOW/index.html', result)
