...
class TestApi:
    ...
    def test_clone_bad_request(self):
        res = self.client().post('/clone', content_type="'application/json'", data=json.dumps(self.bad_clone_data))
    ...
    def test_clone(self, mocks):
        ...
        outgoing_queue_mock.put.assert_called_with(OutgoingMessageGrouped(CLONE, self.repo.encode('utf-8'), b"ungrouped"))

    def test_start_bad_request(self):
        res = self.client().post('/start', content_type="'application/json'", data=json.dumps(self.bad_start_data))
        assert res.status_code == 400
    ...
    def test_stop(self, mocks):
        outgoing_queue_mock = mocks
        res = self.client().post('/stop', content_type="'application/json'", data=json.dumps(self.stop_data))
        assert res.status_code == 200
        outgoing_queue_mock.put.assert_called_with(OutgoingMessageGrouped(STOP_TEST, b"STOP", b"ungrouped"))
    ...
    def test_results(self, mocker):
        ...
        res = self.client().post('/results', content_type="'application/json'", data=json.dumps(self.results_data))
        assert res.status_code == 200
